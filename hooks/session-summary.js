...
try {
  const metricsDir = path.join(homeDir, '.claude', 'logs');
  fs.mkdirSync(metricsDir, { recursive: true });
  const metricsFile = path.join(metricsDir, 'harness-metrics.jsonl');
  ...
  const metrics = {
    ...
    project: projectName,
    directory: cwd,
    files_changed: gitInfo.is_repo ? gitInfo.changes_count : 0,
    files_added: changesDetails.added,
    files_modified: changesDetails.modified,
    files_deleted: changesDetails.deleted
  };

  fs.appendFileSync(metricsFile, JSON.stringify(metrics) + '\n', 'utf8');
} catch {
  // Metrics collection should not block session end
}

// === Harness Self-Evolution: auto-regenerate stale indexes ===
try {
  ...
  if (fs.existsSync(genScript)) {
    let needsRegen = !fs.existsSync(metaPath);

    if (!needsRegen) {
      const metaMtime = fs.statSync(metaPath).mtimeMs;
      const checkDirs = ['skills', 'commands', 'agents'].map(
        d => path.join(homeDir, '.claude', d)
      );
      for (const dir of checkDirs) {
        if (!fs.existsSync(dir)) continue;
        for (const entry of fs.readdirSync(dir)) {
          try {
            if (fs.statSync(path.join(dir, entry)).mtimeMs > metaMtime) {
              needsRegen = true;
              break;
            }
          } catch { /* ignore */ }
    ...
    if (needsRegen) {
      ...
      execSync(`node "${genScript}"`, { timeout: 10000, stdio: 'pipe' });

      // Auto-sync CLAUDE.md inventory
      const claudeMdPath = path.join(homeDir, '.claude', 'CLAUDE.md');
      if (fs.existsSync(metaPath) && fs.existsSync(claudeMdPath)) {
        const meta = JSON.parse(fs.readFileSync(metaPath, 'utf8'));
        let content = fs.readFileSync(claudeMdPath, 'utf8');
        content = content.replace(
          /\| Skills \| .+? \|/,
          `| Skills | ${meta.counts.skills_total} (${meta.counts.skills_local} local + ${meta.counts.skills_plugin} plugin) |`
        );
        content = content.replace(
          /\| Commands \| \d+ \|/,
          `| Commands | ${meta.counts.commands} |`
        );
        content = content.replace(
          /\| Agents \| \d+ \|/,
          `| Agents | ${meta.counts.agents} |`
        );
        fs.writeFileSync(claudeMdPath, content, 'utf8');
      }

      displayMsg += '\n🔄 Harness indexes auto-regenerated\n';
    }
  }
} catch {
  // Self-evolution should never block session end
}
