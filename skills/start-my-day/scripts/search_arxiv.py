...
def load_recommendation_history(history_dir: str) -> Set[str]:
    """
    扫描已有的推荐笔记，提取已推荐过的 arXiv ID

    Args:
        history_dir: 推荐笔记所在目录（如 10_Daily/）
    ...
    if not history_path.exists():
        logger.warning("[History] Directory not found: %s", history_dir)
        return history_ids

    # 匹配所有 *论文推荐.md 文件
    for md_file in sorted(history_path.glob("*论文推荐.md")):
        try:
            with open(md_file, 'r', encoding='utf-8') as f:
                content = f.read()
            # 从 arXiv 链接中提取 ID: arxiv.org/abs/2602.14158v1 -> 2602.14158
            ids = re.findall(r'arxiv\.org/abs/(\d{4}\.\d{4,5})', content)
            history_ids.update(ids)
        except Exception as e:
            logger.warning("[History] Error reading %s: %s", md_file.name, e)

    logger.info("[History] Loaded %d unique paper IDs from %d recommendation notes",
                len(history_ids), len(list(history_path.glob("*论文推荐.md"))))
...
def search_arxiv_by_keywords(
    ...
    """
    使用 arXiv API 按关键词搜索论文（针对高优先级领域的定向搜索）

    Args:
        keywords: 搜索关键词列表（OR 组合）
        start_date: 开始日期
        end_date: 结束日期
        max_results: 最大结果数
        max_retries: 最大重试次数
        categories: 限定 arXiv 分类（可选）
    ...
    keyword_parts = []
    for kw in keywords:
        # Multi-word phrases: use %22 for URL-encoded quotes
        if " " in kw:
            encoded_kw = kw.replace(" ", "+")
            # Search both title and abstract for better recall
            keyword_parts.append(
                f"ti:%22{encoded_kw}%22+OR+abs:%22{encoded_kw}%22"
            )
        else:
            keyword_parts.append(f"ti:{kw}+OR+abs:{kw}")

    # Combine with OR, group each keyword pair
    keyword_query = "+OR+".join(f"({part})" for part in keyword_parts)
    ...
    logger.info("[arXiv-KW] Searching by keywords: %s", ", ".join(keywords[:5]))
    logger.info("[arXiv-KW] URL length: %d chars", len(url))
    ...
    for attempt in range(max_retries):
        try:
            with urllib.request.urlopen(url, timeout=60) as response:
                xml_content = response.read().decode("utf-8")
                papers = parse_arxiv_xml(xml_content)
                logger.info("[arXiv-KW] Found %d papers", len(papers))
                return papers
        except Exception as e:
            logger.warning("[arXiv-KW] Error (attempt %d/%d): %s", attempt + 1, max_retries, e)
            if attempt < max_retries - 1:
                time.sleep((2 ** attempt) * 2)
            else:
                logger.error("[arXiv-KW] Failed after %d attempts", max_retries)
                return []
    return []
...
def calculate_relevance_score(
    ...
    for domain_name, domain_config in domains.items():
        ...
        # 应用领域优先级作为乘数 (priority 1-5 -> multiplier 0.2-1.0)
        priority = domain_config.get('priority', 3)
...
def main():
    ...
    parser.add_argument('--history-dir', type=str, default=None,
                        help='Directory containing previous recommendation notes (*论文推荐.md) for dedup')
    ...
    # ========== 第1.5步：针对高优先级领域的关键词定向搜索 ==========
    logger.info("=" * 70)
    logger.info("Step 1.5: Keyword-targeted search for high-priority domains")
    logger.info("=" * 70)
    ...
    for domain_name, domain_config in domains.items():
        ...
        domain_cats = domain_config.get('arxiv_categories', [])
        if not kws:
            continue
        logger.info("[Step 1.5] Searching domain '%s' (priority=%d) with %d keywords",
                     domain_name, priority, len(kws))
        keyword_papers = search_arxiv_by_keywords(
            keywords=kws,
            start_date=window_30d_start,
            end_date=window_30d_end,
            max_results=args.max_results,
            categories=domain_cats if domain_cats else None
        )
        if keyword_papers:
            ...
            logger.info("  -> Scored %d papers for domain '%s'", len(scored_kw), domain_name)
            all_scored_papers.extend(scored_kw)
        else:
            logger.warning("  -> No papers found for domain '%s'", domain_name)
    ...
    history_ids: Set[str] = set()
    if args.history_dir:
        history_ids = load_recommendation_history(args.history_dir)

    if history_ids:
        before_count = len(unique_papers)
        unique_papers = [
            p for p in unique_papers
            if (p.get('arxiv_id') or '') not in history_ids
        ]
        filtered_count = before_count - len(unique_papers)
        logger.info("[History] Filtered out %d previously recommended papers, %d remaining",
                    filtered_count, len(unique_papers))
    ...
    output = {
        ...
        'history_filtered': len(history_ids),
