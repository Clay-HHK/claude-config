...
# --history-dir 会自动扫描已有推荐笔记，排除已推荐过的论文
cd "$SKILL_DIR"
python scripts/search_arxiv.py \
  ...
  --history-dir "$OBSIDIAN_VAULT_PATH/10_Daily"
```
...
- **其他论文**：只写基本信息，不插入图片
- **保持快速**：让用户快速了解当日推荐
- **历史去重（必须）**：通过 `--history-dir` 参数自动扫描 `10_Daily/` 下所有已有推荐笔记，提取已推荐过的 arXiv ID，在筛选阶段排除，确保每日推荐不会出现重复论文
- **论文笔记时间标记**：
  - 对前3篇论文调用 `paper-analyze` 生成详细报告时，必须在 frontmatter 中添加 `created: YYYY-MM-DD`（笔记创建日期）
  - 在推荐笔记中，对于已有笔记的论文，显示 `created` 日期，方便区分新旧笔记
  - 统一格式：`created` = 笔记创建日期，`date` = 论文发表日期
- **自动关键词链接**：
  - 在生成推荐笔记后，自动扫描现有笔记
